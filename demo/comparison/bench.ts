...
import type { PluginContext } from '@molt/runtime';
import { capability, contributionKey, createRuntime, isMoltError } from '@molt/runtime';
import { fakeResources } from '@molt/test';
...
async function runCordis(): Promise<ScenarioResult> {
  ...
  await fiber.await();
  // Cordis exposes fiber disposal and activation, not Molt's prepare/commit
...
async function runMolt(): Promise<ScenarioResult> {
  ...
  const definition = (version: string, fail = false) => ({
    ...
    setup: async (context: PluginContext) => {
      ...
      if (fail) throw new Error('Molt candidate failure');
    },
  });
...
async function main(): Promise<void> {
  const results = [
    ...
    await measureRunner('cordis', '4.0.0-rc.9', runCordis),
    await measureRunner('@molt/runtime', '0.0.0', runMolt),
  ];
