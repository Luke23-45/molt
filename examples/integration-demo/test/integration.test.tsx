...
describe('integration-demo (real-app, robust, end-to-end)', () => {
  it('INV-06/07/08 + Vite all 3 events + React committed snapshot + observer + dashboard host factory', async () => {
    ...
    // Optional present/absent both succeed INV-09
    expect(runtime.getStatus('demo.optional-consumer')).toBe('active');

    // Diagnostics: storage plugin called ctx.diagnose severity/info INV-10 never replaces error
    ...
    // Observer: throw is isolated, recorded in observerDiagnostics (INV-10), second observer still received events
    ...
    // Contributions are single-owner per key (plan 00 §4.2 step 7) — widget owns widget/route/banner, dashboard owns toast
  ...
  it('INV-01/02/03/04/05/12: Scope LIFO, continue-on-error, idempotent, signal, acquire races, Symbol.asyncDispose, DisposalReport', async () => {
    ...
    // Use pluginHarness for setup-level test (public API only) — harness freezes definition (ADR-06)
    ...
    // Definition identity is preserved; runtime freezes its internal copy (ADR-06) — harness exposes original id
    ...
    let raceScope: import('@molt/runtime').Scope | undefined;
  ...
  it('INV-09/10: capability validation, MoltError 11 codes, BlockedDiagnostic, MISSING/AMBIGUOUS/INCOMPATIBLE/CYCLE', async () => {
    ...
    expect(() =>
      rt.install({
        ...
        version: '1.0.0',
        setup: () => {},
      } as unknown as import('@molt/runtime').PluginDefinition),
    ).toThrow();
    expect(() =>
    ...
    // INV-09: undeclared require/optional/provide/contribute throws with correct codes
  ...
  it('INV-08/14 + INV-15: old disposal failure keeps new, active dependent blocks replace, cascade stop order', async () => {
    ...
    // Also verify no scope was acquired for failed replace (T-R9 asserts zero acquisitions) — old still active
    ...
    // cascade:true succeeds and emits cascade array in order dependents first, provider last (plan 00 §4.4)
  ...
  it('Diagnostics capped at 100, contributions frozen, inspect frozen, reentrancy guard, host provider ordering', async () => {
    ...
    // Diagnostics spam — 150 pushes, only last 100 survive ADR-08
    ...
    // observerDiagnostics also capped at 100 — spam with failing observers
    ...
    // observerDiagnostics message is wrapper "observer threw during ..." + cause is original error
    ...
    // Contributions frozen — direct mutate throws in strict mode or is ignored
  ...
  it('@molt/test exhaustive: fakeResources kinds, create/dispose failures, once:false, clearFailures, counters, expectGenerationDisposed', async () => {
    const resources = fakeResources();
    ...
    // main resources (used for leak checks, not manual) live 0 after disposals — manualResources has extra releases but not used for leak gate
  ...
  it('Stress + queue + coverage: 200 replaces, concurrent per-plugin start serialization, no leaks', async () => {
    const resources = fakeResources();
    const rt = createRuntime();
    // Per-plugin queue: concurrent starts for different plugins serialize per id but not deadlock ADR-04
    const order: string[] = [];
    rt.install({
