/**
 * Public test-host utilities for proving Molt lifecycle ownership.
 * @packageDocumentation
 */

import type { PluginContext, PluginDefinition, Runtime } from '@molt/runtime';
...
/**
 * A resource factory suitable for acquisition through a Molt scope.
 * @public
 */
...
export function pluginHarness(definition: PluginDefinition): PluginHarness {
  ...
  const actualRuntime: Runtime = createRuntime();
  ...
  return {
    ...
    run: () => {
      ...
      running = actualRuntime.start(definition.id).then(() => {
        if (context === undefined) throw new Error('setup did not receive a plugin context');
        return context;
      });
      return running;
    },
    dispose: () => actualRuntime.dispose(),
  };
}
