/**
 * Generation-scoped typed events for Molt hosts and plugins.
 * @packageDocumentation
 */

import type { Capability, Scope } from '@molt/runtime';
...
/**
 * A typed bus whose subscriptions are owned by the supplied Molt scope.
 * @public
 */
...
export function createEventBus<TMap extends EventMap>(
  ...
  const report = (subscriber: Subscriber<unknown>, error: unknown): void => {
    ...
    try {
      ...
      // failure visible to the emitter (adapter form of INV-10).
    }
  };
 ...
 * @throws No Molt errors are produced during factory construction.
 * @public
 */
