// Shared async primitives (plan 03 §9). No domain types live here; scope.ts
// and runtime.ts compose their correctness from these pieces, so each one is
// unit-tested in isolation (plan 04 §2, internal/async.test.ts).
...
/**
 * Per-key operation tails (plan 03 §7 busy policy, ADR-10): operations for a
...
/**
 * Wraps a disposer so it runs exactly once (INV-05 at resource granularity).
...
export function idempotent(disposer: () => void | Promise<void>): () => Promise<void> {
  ...
  return () => {
    ...
    try {
      ...
      // INV-03: the report collects the ORIGINAL thrown value, whatever its
...
/**
 * Ring buffer with a hard capacity (ADR-08): a chatty plugin cannot grow the
