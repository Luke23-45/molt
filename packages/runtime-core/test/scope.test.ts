// Scope and disposal-engine tests (plan 04 §2 — proves INV-01/02/03/04/05/12).
...
// has no dispose() — disposal is the runtime's job (plan 03 §4) — so the
...
describe('abort and protocol integration', () => {
  ...
  it('Symbol.asyncDispose matches dispose() (ADR-09)', async () => {
    const scope = makeScope();
    let calls = 0;
    scope.onDispose(() => {
      calls += 1;
