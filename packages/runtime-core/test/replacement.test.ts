// Replacement-contract tests — written FIRST (note 07 Phase 0), before any
// runtime code exists. Until P1 provides the engine they fail by design
// (plan 04 §7: "a test suite that passes against nothing proves nothing");
// P1-E6 turns them green. Every test asserts the structured MoltError code,
// never a bare throw, and observes the runtime only through public API,
// observer events, inspection, and resource counters.
//
// Sources: note 06 (transaction blockers T-R1…T-R9), plan 03 §7 (lifecycle
// engine), invariants INV-01…INV-15 (plan 00 §2).
...
// @molt/test; it stays inline here so the contract tests are self-contained.
// Counting happens inside the factory/disposer the plugin actually uses —
// counting is observation, never coordination (plan 05 §1).
...
// -----------------------------------------------------------------------------
// The nine transaction gates (note 06, release blockers) plus the two
// ordering tests of ledger P0-B11.

describe('replacement contract (T-R1…T-R9, plan 04 §2)', () => {
  it('T-R1 / INV-01: setup throws after acquiring three resources — all three are disposed, error is ACTIVATION_FAILED', async () => {
  ...
  it('T-R3 / INV-06: failed candidate validation publishes nothing — no capability, no contribution', async () => {
    const counters = makeCounters();
    const runtime = createRuntime();
    runtime.install(storagePlugin(counters));
    await runtime.start('test.provider');
    ...
    const candidate: PluginDefinition = {
      ...
      setup: async (ctx) => {
        ...
        // test.other — commit validation must reject this candidate (03 §6).
    ...
    // candidate scope (INV-01 holds inside the replacement path too).
    expect(counters.counters).toEqual({ acquired: 2, released: 1 });
  });

  it('T-R4 / INV-08 + INV-14: old disposal failure after commit — replacement still succeeds, failure inspectable, no restore', async () => {
    const counters = makeCounters();
    const runtime = createRuntime();
    ...
    if (!isMoltError(error)) {
      throw new Error('expected the DISPOSAL_FAILED diagnostic to be inspectable (INV-14)');
    }
  ...
  it('T-R5 / INV-12: 100 replacements leave resource counters at baseline', async () => {
    const counters = makeCounters();
    const runtime = createRuntime();
    runtime.install(storagePlugin(counters));
    await runtime.start('test.provider');
  ...
  it('T-R6 / INV-11: stopping a provider with active dependents is rejected — zero state change', async () => {
    const counters = makeCounters();
  ...
  it('T-R7 / INV-11: cascade stop — dependents first in deterministic reverse order, cascade recorded', async () => {
    const counters = makeCounters();
    ...
    expect(recordedCascade).toBe(true); // INV-11: which plugins were stopped is recorded
  ...
  it('T-R8 / INV-05: runtime disposal is idempotent — no duplicate disposer calls, no unhandled rejection', async () => {
    const counters = makeCounters();
    const runtime = createRuntime();
    runtime.install(storagePlugin(counters));
    await runtime.start('test.provider');
  ...
  it('T-R9 / INV-15: replacing a provider with active dependents is rejected — zero state change, dependent path named', async () => {
    const counters = makeCounters();
    const { runtime } = await startProviderWithConsumer(counters);
    const capabilitiesBefore = structuredClone(runtime.inspect().capabilities);
  ...
  it('P0-B11a: commit ordering — candidate published and marked active before the old scope is disposed (event sequence)', async () => {
    const counters = makeCounters();
    const log: string[] = [];
    const runtime = createRuntime();
    runtime.subscribe((event) => {
    ...
    // disposal runs — commit is never interleaved with teardown (03 §7).
