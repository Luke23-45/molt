// Runtime engine tests (plan 04 §2 — proves INV-06/09/10/11/13 and the
// note-02 contracts). Complements the replacement-contract gates, which
...
describe('install validation and freezing (ADR-06)', () => {
  ...
  it('freezes the definition at install — mutation fails loudly (ADR-06)', () => {
    const runtime = createRuntime();
  ...
  it('rejects a plugin providing a host-held token with AMBIGUOUS_PROVIDER (plan 00 §4.1)', () => {
    const runtime = createRuntime({
...
describe('start lifecycle and events (INV-06, note 02)', () => {
  ...
  it('a declared provide that is never published → ACTIVATION_FAILED (note 02)', async () => {
    const runtime = createRuntime();
...
describe('contribution ownership (plan 00 §4.2 step 7)', () => {
...
describe('independent instances and observer safety (INV-13, plan 03 §7)', () => {
  ...
  it('provider startup while the provider queue is busy waits on the tail (ADR-04)', async () => {
    const runtime = createRuntime();
    const order: string[] = [];
