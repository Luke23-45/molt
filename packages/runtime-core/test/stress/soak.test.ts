// 10k-operation soak (plan 04 §4): one runtime, observers attached (one of
...
// statement of ADR-08 — diagnostic ring buffers stay at capacity no matter
// how much history the run produces — plus the INV-12 global baseline after
...
describe('ADR-08/INV-12: 10k-operation soak with bounded diagnostics (plan 04 sec 4)', () => {
  it(
    ...
    async () => {
      ...
      runtime.subscribe(() => {
        throw new Error('injected observer failure'); // never breaks outcomes (plan 03 §7)
      });
