// Resolver tests (plan 04 §2 — proves INV-10 and note 04's provider rules
// exhaustively). The resolver is pure: every test constructs definitions,
...
describe('deterministic resolution (note 04 requirement 7)', () => {
...
describe('missing, incompatible, ambiguous, and multi providers (note 04)', () => {
  ...
  it('a multi-provider token selects all compatible providers, host first (note 04 ordering)', () => {
...
describe('version ranges and host providers', () => {
  ...
  it('host providers join the candidate pool - a second single claim is ambiguous', () => {
    ...
    // The runtime rejects this at install (plan 00 sec 4.1); the resolver
...
describe('cycle detection with full paths (note 04 requirement 6)', () => {
  ...
  it('a provider excluded by version selection cannot create a cycle', () => {
    ...
    // part of the selected graph (plan 00 sec 4.1 step 6); other is not in
