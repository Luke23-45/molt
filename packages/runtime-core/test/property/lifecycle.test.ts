// Model-based property suite (plan 04 §3): random worlds and random valid
// operation sequences drive the real runtime; a dumb reference model
// (model.ts) predicts every observable outcome, and the note-06 invariants
// are asserted after EVERY operation:
//
//   active plugin        → exactly one active generation          (INV-05 dual)
//   visible capability   → belongs to an active generation or host (INV-06)
...
//   diagnostic logs      → bounded by the ADR-08 capacity
//
...
function assertInvariants(harness: Harness): void {
  ...
  // INV-12: live resources are exactly the ones owned by active generations.
  ...
  // ADR-08: diagnostic logs are bounded by capacity, never by history.
  for (const plugin of inspection.plugins) {
...
describe('INV-01/05/06/07/11/12: model-based lifecycle properties (plan 04 sec 3)', () => {
  it(
    ...
    async () => {
      const property = fc.asyncProperty(
        ...
        async ({ world, operations }) => {
          ...
          // Terminal dispose: every counter returns to baseline (INV-12,
          // global form — the same statement T-R5 makes for 100 replacements).
