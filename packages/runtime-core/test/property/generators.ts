// World and operation generators for the property suite (plan 04 §3).
//
...
// not generated — "valid operations only", plan 04 §3). Failure injection
...
export interface WorldPlugin {
  ...
  /** Failure injection points (plan 04 §3 arbFailureInjection). */
...
export function arbWorld(): fc.Arbitrary<World> {
  // One policy/host row per pool id, in pool order — canonical per ADR-05.
  ...
  return arbCapabilities.chain((capabilities) => {
    const canProvide = (capabilityId: string): boolean => {
      ...
      // publisher — install would reject the definition (plan 00 §4.1).
...
export interface MintedWorld {
  ...
  /** contribution key id → key (per world; keys resolve by id, ADR-05). */
 ...
 * are canonical per capability id (resolution is keyed by id — ADR-05);
...
export function buildDefinition(
  ...
  const publish = (context: PluginContext): void => {
    if (plugin.failPublish) {
      return; // declared-but-never-published (note 02) — structured failure
    }
 ...
 * validity (plan 04 §3: "valid operations only"; the invalid ones have
