...
Please include: affected package and version, a minimal reproduction, and your assessment of impact. You will receive an acknowledgment within 5 business days and a status update at least every 14 days until resolution.
...
Molt is an **orchestration and ownership library, not a sandbox**. A plugin with JavaScript execution privileges can access any object reachable from its imports or context; Molt does not attempt to isolate untrusted code. This is a documented, deliberate non-goal (`docs/notes/01-thesis-and-boundaries.md`, `docs/notes/05-adapters-and-host-integration.md` § Security).

Therefore the following are **not** security vulnerabilities in Molt:
...
What *is* in scope: a guarantee in `docs/notes/01` / the invariant registry (INV-01…INV-15) failing to hold, transaction boundaries leaking staged state, or disposal semantics that lose ownership of runtime-managed resources.
