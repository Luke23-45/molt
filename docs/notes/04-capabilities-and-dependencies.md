...
The first implementation should support a small, tested subset of semver ranges or depend on one well-defined semver package. It must not implement an ad-hoc comparison such as lexical string sorting.
...
interface ResolutionPlan {
  ...
  providers: ReadonlyMap<string, string | readonly string[]>;
...
Removing a definition that has active dependents is rejected unless the host explicitly requests a cascade. Removing a stopped definition must not delete persisted data owned by it.
