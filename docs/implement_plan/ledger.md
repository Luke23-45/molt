...
- [ ] **P1-E7** Property suite (`test/property/`): `arbWorld` / `arbFailureInjection` / `arbOperations` generators, reference model, INV assertions after every op, seeded determinism — [04 §3](./04-test-plan.md) — _implemented 2026-08-31 (generators, model, model-based runner, resolver permutation property; seeded via `FC_SEED`); intentionally not yet executed per owner instruction — the run/debug session is deferred and the tick waits for a green `pnpm test:property`._
- [ ] **P1-E8** Stress suite: 1,000-definition resolution budget, three-resource-kind replace-100×, 10k-op bounded memory — [04 §4](./04-test-plan.md) — _implemented 2026-08-31 (resolution budget + end-to-end activation, leak zeroing with injected failures and INV-14 windows, 10k soak with throwing observer); not yet executed per owner instruction — the tick waits for a green `pnpm test:stress`._
...
- [ ] **P1-E11** API review file generated and human-reviewed (`*.api.md`) — gate G9 — _`etc/api/runtime.api.md` generated and `check:api` green; surface audited against plan 03 §10 (all 23 public symbols carry JSDoc + `@public` release tags — the gate caught and closed a ground-rule-2 JSDoc gap); the "human-reviewed" half completes at the first PR review_
