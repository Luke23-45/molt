...
### `release.yml` — triggered by the changesets version PR

1. `pnpm verify` + full gate suite re-run (never release from a stale green).
2. `changesets publish` → npm with **provenance** (OIDC-linked builds; `id-token: write` permission, `NPM_CONFIG_PROVENANCE=true`).
3. Git tags `@molt/runtime@x.y.z` per package; GitHub Release per package with its changeset changelog.
4. Regenerate `demo/comparison/RESULTS.md` on the release branch and commit (the published benchmark can never go stale silently — [02 §4](./02-tooling-and-quality-gates.md)).

npm organization `@molt` is claimed before the first publish (note [08](../notes/08-positioning-and-related-work.md) naming record); automation token or trusted publishing is configured at P1, not on release day.
...
2. Two real host adapters or example hosts → `examples/command-host`, `examples/worker-host` (P2), plus Sky (P4).
